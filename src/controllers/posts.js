...
const { calculateReadingTime } = require('../utils/calcReadTime');
...
const updatePost = async (req, res) => {
  try {
    ...
    const post = await Posts.findByIdAndUpdate(req.params.id, postData, {
      new: true,
    });
...
const deletePost = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ message: `(${post.title})post deleted sucessfully` });
  } catch (error) {
    logger.error(error);
    res.status(500).json({ error: 'Internal Server Error' });
  }
